...
def log_progress(epoch: int, total_epochs: int, metrics: Dict[str, float], phase: str = "training"):
    """
    Output progress in JSON format for API parsing.
    Format: {"type": "progress", "epoch": X, "total": Y, "metrics": {...}, "phase": "..."}
    """
    progress_data = {
        "type": "progress",
        "epoch": epoch,
...
def log_status(status: str, message: str, data: Optional[Dict] = None):
    ...
    Format: {"type": "status", "status": "...", "message": "...", ...}
    """
    status_data = {
        "type": "status",
        "status": status,
...
def main(cfg: DictConfig):
    ...
    log_status("initializing", "Loading data and preparing model", {
    ...
    try:
        ...
        log_status("data_loaded", f"Loaded {len(train_dl)} train batches, {len(val_dl)} val batches")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_loss < best_val:
            ...
            model_path = os.path.join(out_dir, f"{cfg.data.ticker}_model.pth")
            torch.save({
                'epoch': epoch,
        ...
        if no_improve >= early_patience:
            print(f"\nEarly stopping at epoch {epoch} (no improvement for {early_patience} epochs)", flush=True)
            log_status("early_stopping", f"Stopped at epoch {epoch}", {"best_val_loss": round(best_val, 6)})
            break
    ...
    # Сохраняем скейлеры
    scaler_X_path = os.path.join(out_dir, f"{cfg.data.ticker}_scaler_X.pkl")
    scaler_y_path = os.path.join(out_dir, f"{cfg.data.ticker}_scaler_y.pkl")
    ...
    try:
        ...
        # Финальный статус
        log_status("completed", "Training completed successfully", {
            "ticker": cfg.data.ticker,
            "version": version,
            "model_path": out_dir,
            "final_metrics": {
...
if __name__ == "__main__":
    try:
        ...
        log_status("error", f"Training failed: {str(e)}", {"error_type": type(e).__name__})
        raise