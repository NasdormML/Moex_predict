...
def predict(request: PredictionRequest):
    ...
    with mlflow.start_run(run_name=f"Predict_{ticker}_{datetime.now():%Y%m%d_%H%M%S}"):
        ...
        last_train = (
            datetime.strptime(last_train_str, "%Y-%m-%d")
            if last_train_str
            else datetime.min
        )
        ...
        df_t = fetch_moex_eod_data(
            ticker, "stock", "shares", "TQBR", request.start_date, request.end_date
        )
        df_i = fetch_moex_eod_data(
            "IMOEX", "stock", "index", "SNDX", request.start_date, request.end_date
        )
